...
pub const TSInput = extern struct {
    payload: ?*anyopaque,
    read: ?fn (?*anyopaque, u32, TSPoint, [*c]u32) callconv(.C) [*c]const u8,
...
pub const TSLogger = extern struct {
    payload: ?*anyopaque,
    log: ?fn (?*anyopaque, TSLogType, [*c]const u8) callconv(.C) void,
};
...
pub extern fn ts_set_allocator(new_malloc: ?fn (usize) callconv(.C) ?*anyopaque, new_calloc: ?fn (usize, usize) callconv(.C) ?*anyopaque, new_realloc: ?fn (?*anyopaque, usize) callconv(.C) ?*anyopaque, new_free: ?fn (?*anyopaque) callconv(.C) void) void;
